...
INPUT_CSV = pathlib.Path("/Users/cataivancov/IdeaProjects/arke-platform/data/teluk-wondama-rumberpon-roon.csv")

COLLECTION_NAME = "Teluk Wondama - Papua: Rumberpon and Roon islands Cluster"
DATASET_NAME = COLLECTION_NAME
DATASET_DESCRIPTION = (
    "Thematic dataset of archaeological sites in Teluk Wondama Regency (Rumberpon and Roon islands, West Papua), "
    "compiled from Mas'ud (2019)."
)
...
DATASET_SOURCE_DESCRIPTION = "Local CSV import (Teluk Wondama / Rumberpon / Roon)"
DATASET_SOURCE_RELATION = ""
DATASET_SUBJECT = "teluk wondama; rumberpon; roon; west papua; rock art; burial"
DATASET_COPYRIGHT = "Local working copy"
DATASET_REUSE = "Local evaluation import only."
DATASET_GEOGRAPHICAL_LIMIT = "Teluk Wondama, Rumberpon, and Roon (West Papua, Indonesia)"
DATASET_CONTEXT_DESCRIPTION = "Thematic subset of sites in Teluk Wondama Regency"
...
def build_dataset(rows, charac_paths):
    ...
    for row in rows:
        ...
        knowledge_type = KNOWLEDGE_TYPE_MAP.get((row.get("STATE_OF_KNOWLEDGE") or "").strip(), "not_documented")
        occupation = OCCUPATION_MAP.get((row.get("OCCUPATION") or "").strip(), "not_documented")
        locality = row["LOCALISATION"].strip() or row["SITE_NAME"].strip()
        site = {
...
def main():
    ...
    for db in DATABASES:
        ...
        print(f"[teluk-wondama:{db}] sites={len(dataset['sites'])} dataset_id={dataset_id}")
