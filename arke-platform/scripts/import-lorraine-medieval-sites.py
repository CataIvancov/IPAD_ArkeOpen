...
CSV_URL = "https://api.nakala.fr/data/10.34847/nkl.db53j42l/3198b9d7a95945bff5833019c71efdd77f08250c"
DATASET_NAME = "Sites archeologiques de la Lorraine medievale"
DATASET_DESCRIPTION = (
    "L'Itineraire Medieval des sites Archeologiques du Grand Est (I.M.A.G.E.) est "
    "une base de donnees iconographique produite par le laboratoire Histoire et "
    "Cultures de l'Antiquite et du Moyen Age de l'Universite de Lorraine."
)
DATASET_BIBLIOGRAPHY = "Charlene MOREL. Sites archeologiques de la Lorraine medievale. DOI 10.34847/nkl.db53j42l."
DATASET_SOURCE_DESCRIPTION = "Import CSV Nakala AKG5_IMAGE_Med_Lorraine.csv"
DATASET_SOURCE_RELATION = "https://nakala.fr/10.34847/nkl.db53j42l"
DATASET_SUBJECT = "inventaire; Lorraine; Moyen Age; motte; chateau; photographie"
DATASET_COPYRIGHT = "CC-BY-NC-ND-4.0"
DATASET_REUSE = "Consultation et reutilisation soumises a la licence CC-BY-NC-ND-4.0."
DATASET_GEOGRAPHICAL_LIMIT = "Lorraine, Grand Est, France"
DATASET_CONTEXT_DESCRIPTION = "Inventaire iconographique medieval"
...
def build_dataset(rows, charac_paths):
    ...
    for row in rows:
        locality = row["LOCALISATION"].strip()
    ...
    for row in rows:
        ...
        site = {
            ...
            "city_name": row["LOCALISATION"].strip(),
            "city_geonameid": city_map[row["LOCALISATION"].strip()]["geonameid"],
            "longitude": float(row["LONGITUDE"]),
            "latitude": float(row["LATITUDE"]),
            "altitude": altitude,
    ...
    return {
        ...
        "sites": sites,
        "start_date": min(site["start_date1"] for site in sites if site["start_date1"] != 0),
        "end_date": max(site["end_date2"] for site in sites if site["end_date2"] != 0),
    }
...
def build_sql(dataset_id, license_id, next_range_id, next_src_id, dataset):
    ...
    statements.extend(
        [
            ...
            f"{sql_literal('Universite de Lorraine')}, {sql_literal('https://www.univ-lorraine.fr/')}, {sql_literal('Charlene MOREL')}, 'fr', 'finished', "
            f"{license_id}, true, false, ST_GeogFromText({sql_literal(bbox_wkt)}), {dataset['start_date']}, {dataset['end_date']}, "
            f"{sql_literal('2015-01-01T00:00:00+00:00')}, true, now(), now(), {ROOT_CHRONOLOGY_ID}, '', 0) "
...
def verify(db, dataset_id):
    ...
    site_query = (
        "SELECT s.id, s.name, s.city_name, s.start_date1, s.end_date2, src.charac_id "
        "FROM public.site s "
        "JOIN public.site_range sr ON sr.site_id = s.id "
        "JOIN public.site_range__charac src ON src.site_range_id = sr.id "
        f"WHERE s.database_id = {dataset_id} ORDER BY s.id;"
    )
    print(f"[lorraine-sites:{db}]")
    print(sh(["docker", "exec", CONTAINER, "psql", "-U", "postgres", "-d", db, "-Atc", query]).stdout, end="")
    print(sh(["docker", "exec", CONTAINER, "psql", "-U", "postgres", "-d", db, "-Atc", site_query]).stdout, end="")
