...
CSV_PATH = ROOT / "data" / "indonesia-prehistory-protohistory-chronology-en.csv"
...
ROOT_CREDITS = (
    "Draft chronology assembled from user-supplied screenshots and the local Google Drive PDF corpus "
    "for Indonesian prehistory and protohistory."
)
...
def build_sql(rows):
    ...
    statements.extend(
        [
            ...
            f"VALUES ({root_id}, 0, 0, {sql_literal(ROOT_CREDITS)}, true, ST_GeogFromText({sql_literal(INDONESIA_POLYGON)}), 'en', false, "
...
def verify(db, root_id):
    ...
    print(f"[indonesia-chronology:{db}]")
