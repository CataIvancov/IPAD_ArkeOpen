...
OUTPUT_CSV = BASE_DIR / "data" / "airtable-to-arkeogis-v3-site-only.csv"
REPORT_TXT = BASE_DIR / "data" / "airtable-to-arkeogis-v3-site-only-report.txt"
...
def classify_row(row: dict[str, str], unresolved: Counter[str], excluded: Counter[str]) -> tuple[str, str, str, str, str]:
...
def build_output_rows(rows: list[dict[str, str]]) -> tuple[list[dict[str, str]], Counter[str], Counter[str], Counter[str]]:
    output_rows = []
    ...
    for row in rows:
        ...
        main_charac, lvl1, lvl2, lvl3, lvl4 = classify_row(row, unresolved_charac, excluded_tokens)
    ...
    return output_rows, unresolved_chronology, unresolved_charac, excluded_tokens
...
def write_report(
    ...
    unresolved_chronology: Counter[str],
    unresolved_charac: Counter[str],
    excluded_tokens: Counter[str],
) -> None:
    type_counter = Counter(row["MAIN_CHARAC"] for row in rows)
    lines = [
        "Airtable -> ArkeoGIS v3 site-only conversion report",
        "",
...
def main() -> None:
    ...
    output_rows, unresolved_chronology, unresolved_charac, excluded_tokens = build_output_rows(rows)
    write_csv(output_rows)
    write_report(output_rows, unresolved_chronology, unresolved_charac, excluded_tokens)
