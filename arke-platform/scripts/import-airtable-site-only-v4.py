...
INPUT_CSV = pathlib.Path("/Users/cataivancov/IdeaProjects/arke-platform/data/airtable-to-arkeogis-v4-site-only.csv")

DATASET_NAME = "Airtable archaeology sites (site-only v4)"
DATASET_DESCRIPTION = (
    "Site-level import generated from a local Airtable export. "
    "Object/find-specific subtype values were excluded so the dataset fits the current ArkeOpen site model."
)
DATASET_BIBLIOGRAPHY = "Local Airtable export transformed to ArkeoGIS CSV v4."
DATASET_SOURCE_DESCRIPTION = "Local Airtable CSV transformed into ArkeoGIS import template"
DATASET_SOURCE_RELATION = ""
DATASET_SUBJECT = "airtable; archaeology; sites; site-only"
DATASET_COPYRIGHT = "Local working copy"
DATASET_REUSE = "Local evaluation import only."
DATASET_GEOGRAPHICAL_LIMIT = "Mixed localities from Airtable export"
DATASET_CONTEXT_DESCRIPTION = "Site-level harmonised import"
...
def build_dataset(rows, charac_paths):
    ...
    min_lon = min(float(row["LONGITUDE"]) for row in rows if row["LONGITUDE"])
    min_lat = min(float(row["LATITUDE"]) for row in rows if row["LATITUDE"])
    max_lon = max(float(row["LONGITUDE"]) for row in rows if row["LONGITUDE"])
    max_lat = max(float(row["LATITUDE"]) for row in rows if row["LATITUDE"])

    for row in rows:
        locality = row["LOCALISATION"].strip() or row["SITE_NAME"].strip()
...
def get_ids(db):
    ...
    license_id = int(query_value(db, f"SELECT id FROM public.license WHERE name = {sql_literal(LICENSE_NAME)} OR url = {sql_literal(LICENSE_URL)} ORDER BY id LIMIT 1;"))
...
def build_sql(dataset_id, license_id, next_range_id, next_src_id, dataset):
    ...
    statements.extend([
        ...
        f"{sql_literal('Local Airtable import')}, {sql_literal('')}, {sql_literal('Local transform')}, 'en', 'finished', "
        f"{license_id}, true, false, ST_GeogFromText({sql_literal(bbox_wkt)}), {dataset['start_date']}, {dataset['end_date']}, "
        f"{sql_literal('2026-03-21T00:00:00+00:00')}, true, now(), now(), {ROOT_CHRONOLOGY_ID}, '', 0) "
...
def verify(db, dataset_id):
    query = (
        "SELECT d.id, d.name, count(s.id), min(CASE WHEN s.start_date1 != -2147483648 THEN s.start_date1 END), "
        "max(CASE WHEN s.end_date2 != 2147483647 THEN s.end_date2 END) "
        "FROM public.database d LEFT JOIN public.site s ON s.database_id = d.id "
        f"WHERE d.id = {dataset_id} GROUP BY d.id, d.name;"
    )
    sample_query = (
        "SELECT s.id, s.name, src.charac_id "
        "FROM public.site s "
        "JOIN public.site_range sr ON sr.site_id = s.id "
        "JOIN public.site_range__charac src ON src.site_range_id = sr.id "
        f"WHERE s.database_id = {dataset_id} ORDER BY s.id LIMIT 10;"
    )
    print(f"[airtable-v4:{db}]")
    print(sh(["docker", "exec", CONTAINER, "psql", "-U", "postgres", "-d", db, "-Atc", query]).stdout, end="")
    print(sh(["docker", "exec", CONTAINER, "psql", "-U", "postgres", "-d", db, "-Atc", sample_query]).stdout, end="")
